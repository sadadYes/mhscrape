...
def setup_get(client):
    ...
    async def search(interaction: discord.Interaction, query: str):
        ...
        search_url = f"https://api-pddikti.kemdiktisaintek.go.id/pencarian/mhs/{
            query}"
        ...
        if data and 'id' in data[0]:
            ...
            detail_url = f"https://api-pddikti.kemdiktisaintek.go.id/detail/mhs/{
                student_id}"
            ...
            embed = discord.Embed(
                title="Informasi Mahasiswa", color=discord.Color.blue())
            embed.add_field(name="Nama Mahasiswa", value=nama, inline=False)
            embed.add_field(name="Jenis Kelamin",
                            value=jenis_kelamin, inline=False)
            embed.add_field(name="Perguruan Tinggi",
                            value=nama_pt, inline=False)
            embed.add_field(name="Tahun Masuk",
                            value=tahun_masuk, inline=False)
            embed.add_field(name="Jenjang - Program Studi",
                            value=f"{jenjang} - {prodi}", inline=False)
            embed.add_field(name="Nomor Induk Mahasiswa",
                            value=nim, inline=False)
            embed.add_field(name="Status Awal Mahasiswa",
                            value=jenis_daftar, inline=False)
            embed.add_field(name="Status Akhir Mahasiswa",
                            value=status_saat_ini, inline=False)
